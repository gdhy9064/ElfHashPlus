...
#endif
{
  ...
  //https://lists.debian.org/lsb-spec/1999/12/msg00017.html
	...
	ElfXX_Shdr* pgb_shdr = elf_get_section_bytype(base, SHT_PROGBITS);
	if(pgb_shdr == NULL){
	  printf("not found section SHT_PROGBITS\n");
	}else{
	  printf("zzz dump SHT_GNU_verdef offset=0x%x, size=%d, entsize=%d\n", pgb_shdr->sh_offset, pgb_shdr->sh_size, pgb_shdr->sh_entsize);
	}
